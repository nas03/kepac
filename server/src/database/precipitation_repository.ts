...
export const uploadGeoTIFFFile = async (payload: PrecipitationRepo | PrecipitationRepo[]) => {
...
export const getGeoTIFFFile = async (time: string) => {
  const query = await db("precipitation")
    ...
    .andWhereRaw("EXTRACT(HOUR FROM birthtime) = EXTRACT(HOUR FROM ?::timestamp)", [time])
    .limit(1)
    .select("*")
    .first();
  return query;
